...
export const correctionAnalyseSchema = z.object({
  ...
  // ⚠ La chaîne vide est testée EN PREMIER : `z.coerce.number()` transforme ""
  // en 0, et 0 est un degré valide — vider le champ aurait écrit « 0 % » au lieu
  // d'effacer la valeur.
  millesime: z.union([z.literal(""), z.coerce.number().int().min(1900).max(2100)]).optional(),
  degre: z.union([z.literal(""), z.coerce.number().min(0).max(25)]).optional(),
