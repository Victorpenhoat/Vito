...
export function ReservationForm({ voyageId }: { voyageId: string }) {
  ...
  return (
    <form action={action} data-testid="reservation-form" className="flex flex-col gap-2 border-t border-line pt-3">
      ...
      <select name="type" aria-label={t("type")} className="rounded-xl border border-line bg-surface px-3 py-2 outline-none focus:outline-2 focus:outline-accent" defaultValue="hotel">
        {RESERVATION_TYPES.map((ty) => <option key={ty} value={ty}>{t(`types.${ty}`)}</option>)}
      </select>
      <input name="fournisseur" placeholder={t("fournisseur")} className="rounded-xl border border-line bg-surface px-3 py-2 outline-none focus:outline-2 focus:outline-accent" />
      <input name="reference" placeholder={t("reference")} className="rounded-xl border border-line bg-surface px-3 py-2 outline-none focus:outline-2 focus:outline-accent" />
      ...
      <input name="conciergerieTel" placeholder={t("conciergerieTel")} className="rounded-xl border border-line bg-surface px-3 py-2 outline-none focus:outline-2 focus:outline-accent" />
