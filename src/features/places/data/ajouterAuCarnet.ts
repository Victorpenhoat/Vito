...
type Origine = { type: "voyage"; qui: string | null };
...
export async function ajouterAuCarnet(
  ...
  const origine = opts.origine
    ? { origine_type: opts.origine.type, origine_qui: opts.origine.qui }
    : {};

  const { error: itemErr } = await supabase
    .from("liste_items")
    .upsert(
      { user_id: userId, etablissement_id: etabId, ...statut, ...origine },
      { onConflict: "user_id,etablissement_id", ignoreDuplicates: opts.origine != null },
    );
  if (itemErr) return { error: "Ajout à la liste échoué" };

  return { etablissementId: etabId as string };
}
