...
export function GroupeForm({ voyages }: { voyages: VoyageOption[] }) {
  ...
  return (
    <form action={action} data-testid="groupe-form" className="flex flex-col gap-2 max-w-md">
      ...
      <input name="devise" defaultValue="EUR" maxLength={3} aria-label={t("devise")} className="rounded-control border border-line bg-surface px-3 py-2 outline-none focus:outline-2 focus:outline-accent" />
      <select name="voyageId" aria-label={t("voyageLie")} className="rounded-control border border-line bg-surface px-3 py-2 outline-none focus:outline-2 focus:outline-accent" defaultValue="">
