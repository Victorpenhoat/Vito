...
export function DocumentRow({ doc, selected, onSelect }: { doc: DocMeta; selected?: boolean; onSelect?: () => void }) {
  ...
  return (
    <li
      ...
      className={`flex items-center gap-3 rounded-card border border-line bg-surface p-3 ${onSelect ? "cursor-pointer" : ""} ${selected ? "ring-2 ring-accent" : ""}`}
      onClick={onSelect}
