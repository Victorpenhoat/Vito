...
import { getCave, getVinsConnus } from "@/features/vins/data/queries";
...
export default async function RestosPage() {
  ...
  const [places, archived, tags, cave, vinsConnus, tagsVin] = await Promise.all([
    getPlaces("resto"),
  ...
  return (
    <main className="flex flex-col gap-6 p-4 md:p-8 lg:mx-auto lg:w-full lg:max-w-[1200px]">
      ...
      <CategoryTabs places={places} archived={archived} tags={tags} categorie="resto"
        ongletSupplementaire={{ slug: "cave", contenu: <CavePanel vins={cave} vinsConnus={vinsConnus} tags={tagsVin} /> }} />
    </main>
  );
}
